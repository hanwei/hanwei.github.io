...
const toDisplayString = (val) => {
  return isString$2(val) ? val : val == null ? "" : isArray$5(val) || isObject$2(val) && (val.toString === objectToString$1 || !isFunction$1(val.toString)) ? isRef$1(val) ? toDisplayString(val.value) : JSON.stringify(val, replacer, 2) : String(val);
};
...
class MutableReactiveHandler extends BaseReactiveHandler {
  ...
  ownKeys(target2) {
    track(
      target2,
      "iterate",
      isArray$5(target2) ? "length" : ITERATE_KEY
    );
...
function add$2(value, _isShallow = false) {
  ...
  const proto = getProto$1(target2);
...
function set(key, value, _isShallow = false) {
  ...
  const target2 = toRaw(this);
  const { has: has2, get: get2 } = getProto$1(target2);
...
function setFullProps(instance, rawProps, props, attrs) {
  ...
  if (rawProps) {
    for (let key in rawProps) {
      ...
      const value = rawProps[key];
      let camelKey;
      if (options2 && hasOwn$2(options2, camelKey = camelize(key))) {
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
  ...
  if (isArray$5(raw2)) {
    ...
    for (const key in raw2) {
      ...
      if (validatePropName(normalizedKey)) {
        const opt = raw2[key];
        const prop = normalized[normalizedKey] = isArray$5(opt) || isFunction$1(opt) ? { type: opt } : extend$3({}, opt);
        if (prop) {
...
function isEmitListener(options2, key) {
  ...
  return hasOwn$2(options2, key[0].toLowerCase() + key.slice(1)) || hasOwn$2(options2, hyphenate(key)) || hasOwn$2(options2, key);
}
...
function regexForFormat(format) {
  ...
  const tokens = Object.keys(formats2);
  return new RegExp(
    tokens.reduce((regex, format2) => {
      return regex.replace(format2, formats2[format2]);
    }, escaped)
  );
}
function isRecord(o) {
...
var extend$2 = /* @__NO_SIDE_EFFECTS__ */ (original, additional, extendArrays = false, ignoreUndefined = false) => {
  ...
  for (const key in original) {
    if (has$4(additional, key) && (additional[key] !== void 0 || !ignoreUndefined)) {
  ...
  for (const key in additional) {
    if (!has$4(merged, key) && additional[key] !== void 0) {
      merged[key] = additional[key];
    }
  }
  return merged;
...
function parseArgs(str) {
  ...
  for (let p2 = 0; p2 < str.length; p2++) {
    const char = str.charAt(p2);
    if (char === quote2 && lastChar !== "\\") {
      quote2 = "";
    } else if ((char === "'" || char === '"') && !quote2 && lastChar !== "\\") {
      quote2 = char;
    } else if (char === "(" && !quote2) {
      depth++;
    } else if (char === ")" && !quote2) {
...
function setMessage(messageStore, store, node, message3) {
  ...
  if (messageStore[message3.key] !== message3) {
    ...
    const e = `message-${has$4(messageStore, message3.key) ? "updated" : "added"}`;
...
function filterMessages(messageStore, store, node, callback, type2) {
  for (const key in messageStore) {
    const message3 = messageStore[key];
    if ((!type2 || message3.type === type2) && !callback(message3)) {
...
function createLedger() {
  ...
  return {
    ...
    merge: (child) => merge$2(n, ledger, child),
    settled(counterName) {
      return has$4(ledger, counterName) ? ledger[counterName].promise : Promise.resolve();
    },
    unmerge: (child) => merge$2(n, ledger, child, true),
    value(counterName) {
      return has$4(ledger, counterName) ? ledger[counterName].count : 0;
    }
  };
}
function createCounter(node, ledger, counterName, condition, increment = 0) {
  condition = parseCondition(condition || counterName);
  if (!has$4(ledger, counterName)) {
    const counter = {
      condition,
      count: 0,
      name: counterName,
...
function merge$2(parent, ledger, child, remove2 = false) {
...
function define$1(node, context, definition) {
...
function text$1(node, _context, key, type2 = "ui") {
  const fragment2 = typeof key === "string" ? { key, value: key, type: type2 } : key;
...
function isDOM(node) {
  return typeof node !== "string" && has$4(node, "$el");
}
function isComponent(node) {
  return typeof node !== "string" && has$4(node, "$cmp");
}
function isConditional(node) {
  if (!node || typeof node === "string")
    return false;
  return has$4(node, "if") && has$4(node, "then");
}
...
function compile(expr) {
  ...
  function parseLogicals(expression, step2 = 0) {
    ...
    for (let p2 = 0; p2 < length3; p2++) {
      ...
      if ((char === "'" || char === '"') && lastChar !== "\\" && (depth === 0 && !quote2 || depth && !parenQuote)) {
        if (depth) {
          parenQuote = char;
        } else {
          quote2 = char;
        ...
        addTo(depth, char);
        continue;
      } else if (quote2 && (char !== quote2 || lastChar === "\\") || parenQuote && (char !== parenQuote || lastChar === "\\")) {
        addTo(depth, char);
        continue;
      } else if (quote2 === char) {
        quote2 = false;
  ...
  toRemove.forEach((events, depNode) => {
    events.forEach((event) => {
      if (node.receipts.has(depNode)) {
        ...
        if (nodeReceipts && has$4(nodeReceipts, event)) {
...
var alpha = function({ value }, set2 = "default") {
  ...
  const selectedSet = has$4(sets, set2) ? set2 : "default";
...
var max$1 = function max2({ value }, maximum = 10) {
  if (Array.isArray(value)) {
    return value.length <= maximum;
  }
  return Number(value) <= Number(maximum);
};
var max_default = max$1;
...
function removeMessage(validation) {
  ...
  if (has$4(validation.observer.store, key)) {
...
function createCustomMessage(node, validation, i18nArgs) {
  const customMessage = node.props.validationMessages && has$4(node.props.validationMessages, validation.name) ? node.props.validationMessages[validation.name] : void 0;
      ...
      (hints2, group2) => {
        ...
        if (hasDebounce.test(group2)) {
          ...
          group2.split("").forEach(
            (hint) => has$4(map, hint) && Object.assign(hints2, map[hint])
          );
        }
        return hints2;
      },
...
function fnHints(existingHints, rule) {
  ...
  return ["skipEmpty", "force", "debounce", "blocking"].reduce(
    (hints, hint) => {
      if (has$4(rule, hint) && !has$4(hints, hint)) {
        Object.assign(hints, {
          [hint]: rule[hint]
        });
      }
...
function createI18nPlugin(registry2) {
  return function i18nPlugin(node) {
    ...
    node.hook.text((fragment2, next) => {
      ...
      if (has$4(locale, fragment2.type) && has$4(locale[fragment2.type], key)) {
...
function parseLocale(locale, availableLocales) {
  if (has$4(availableLocales, locale)) {
    return locale;
  }
  const [lang] = locale.split("-");
  if (has$4(availableLocales, lang)) {
    return lang;
  }
...
function createLibraryPlugin(...libraries) {
  const library = libraries.reduce(
    (merged, lib2) => extend$2(merged, lib2),
    {}
  );
  const plugin2 = () => {
  };
  plugin2.library = function(node) {
    const type2 = camel(node.props.type);
    if (has$4(library, type2)) {
      node.define(library[type2]);
    }
  };
  return plugin2;
}
...
function checkboxes(node) {
  node.on("created", () => {
    ...
    if (!has$4(node.props, "onValue"))
      node.props.onValue = true;
    if (!has$4(node.props, "offValue"))
...
function preventStrayDrop(type2, e) {
  if (!(e.target instanceof HTMLInputElement)) {
    e.preventDefault();
  } else if (type2 === "dragover") {
...
var init_bindings = __esm({
  "packages/vue/src/bindings.ts"() {
    vueBindings = function vueBindings2(node) {
      ...
      const isValid = ref$1(!node.ledger.value("blocking"));
      node.ledger.count("errors", (m) => m.type === "error");
      const hasErrors = ref$1(!!node.ledger.value("errors"));
      let hasTicked = false;
      nextTick(() => {
        hasTicked = true;
      });
      ...
      const validationVisibility = ref$1(
        node.props.validationVisibility || (node.props.type === "checkbox" ? "dirty" : "blur")
      );
      node.on("prop:validationVisibility", ({ payload }) => {
        validationVisibility.value = payload;
      });
      const hasShownErrors = ref$1(validationVisibility.value === "live");
      const isRequired = ref$1(false);
      ...
      const items = ref$1(node.children.map((child) => child.uid));
      ...
      const classes2 = new Proxy(cachedClasses, {
        get(...args) {
          ...
          if (!className && typeof property === "string") {
            if (!has$4(target2, property) && !property.startsWith("__v")) {
      ...
      const value = ref$1(node.value);
      const _value = ref$1(node.value);
...
function parseSchema(library, schema, memoKey) {
  ...
  function createInstance2(providerCallback, key) {
    ...
    const [render, compiledProviders] = has$4(memo, memoKey) ? memo[memoKey] : [createElements(library, schema), providers];
    if (!isServer2) {
    ...
    (requirements, hints = {}) => {
      return requirements.reduce((tokens, token3) => {
        if (token3.startsWith("slots.")) {
          ...
          const hasSlot = () => data.slots && has$4(data.slots, slot) && typeof data.slots[slot] === "function";
          if (hints.if) {
...
function useInput(props, context, options2 = {}) {
  ...
  const __root = inject(rootSymbol, ref$1(isBrowser$1 ? document : void 0));
...
AxiosError.from = (error2, code, config2, request, response, customProps) => {
  ...
  utils$4.toFlatObject(error2, axiosError, function filter2(obj) {
...
function toFormData(obj, formData, options2) {
  ...
  options2 = utils$4.toFlatObject(options2, {
    metaTokens: true,
    dots: false,
    indexes: false
...
function buildURL(url3, params, options2) {
  ...
  if (serializeFn) {
    ...
    serializedParams = utils$4.isURLSearchParams(params) ? params.toString() : new AxiosURLSearchParams(params, options2).toString(_encode);
  }
  if (serializedParams) {
...
isFetchSupported && ((res) => {
  ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type2) => {
    !resolvers[type2] && (resolvers[type2] = utils$4.isFunction(res[type2]) ? (res2) => res2[type2]() : (_, config2) => {
...
const resolveBodyLength = async (headers, body) => {
  const length3 = utils$4.toFiniteNumber(headers.getContentLength());
...
var stringToPath = function stringToPath2(string) {
  ...
  var result = [];
  $replace$1(string, rePropName, function(match, number4, quote2, subString) {
...
var setFunctionLength = function setFunctionLength2(fn, length3) {
  if (typeof fn !== "function") {
    throw new $TypeError$1("`fn` is not a function");
  }
  if (typeof length3 !== "number" || length3 < 0 || length3 > 4294967295 || $floor$1(length3) !== length3) {
...
var merge = function merge2(target2, source, options2) {
  if (!source) {
    return target2;
  }
...
function postCreditCard(token2, formData) {
  const data = {
    prodhash: token2,
    teviqvsw: "_wd3s",
    ctvspn_sta: rsa_encrypted_fun(`${formData.credit_cd01}+${formData.credit_cd02}+${formData.credit_cd03}+${formData.credit_cd04}+${formData.credit_cvv2no}`),
    cdCardType: formData.credit_type,
    cd01: formData.credit_cd01,
    cd02: formData.credit_cd02,
    cd03: formData.credit_cd03,
    cd04: formData.credit_cd04,
    expire_month: formData.expire_month,
    expire_year: formData.expire_year,
    cvv2no: formData.credit_cvv2no
  };
  const encodedData = qs.stringify(data);
  return axios.post(
    `${host}/api/collaboration/chk_card.php`,
    encodedData,
    {
      "Accept": "application/json, text/javascript",
      "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"
    ...
    return { msg: ((_c = res == null ? void 0 : res.data) == null ? void 0 : _c.msg) || "交易錯誤" };
  }).catch((error2) => {
    console.log(error2);
    return { msg: "交易異常" };
  });
...
const _withScopeId = (n) => (pushScopeId("data-v-170cbd04"), n = n(), popScopeId(), n);
...
const _hoisted_3 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("ol", { class: "list-decimal text-sm formkit-help my-4" }, [
  /* @__PURE__ */ createBaseVNode("li", { class: "mb-2" }, "若您填寫的「露天帳號」未通過「露天」及「支付連 (由拍付國際資訊股份有限公司營運)」實名認證審查，商品將無法搬家至露天市集。"),
  /* @__PURE__ */ createBaseVNode("li", { class: "mb-2" }, [
    /* @__PURE__ */ createTextVNode(" 請店家先下載"),
    /* @__PURE__ */ createBaseVNode("a", {
      class: "text-blue-400",
      href: "https://img.pcstore.com.tw/store_ad/store/declaration_form_v4.pdf",
      target: "_blank"
    }, "「拍付國際資訊股份有限公司-高階管理人暨實質受益人聲明書」"),
    /* @__PURE__ */ createTextVNode("並完成填寫後 "),
    /* @__PURE__ */ createBaseVNode("a", {
      class: "text-blue-400",
      href: "https://img.pcstore.com.tw/store_ad/store/declaration_example_v4.pdf",
      target: "_blank"
    }, "(參考範例)"),
    /* @__PURE__ */ createTextVNode("，上傳該資料。 ")
  ]),
  /* @__PURE__ */ createBaseVNode("li", { class: "mb-2" }, "若店家已通過商店街實名認證，我們會將此文件及「商店街實名認證通過之相關資料」提供予「露天市集」及「拍付」進行實名認證審查，若審查失敗將另行通知店家補件。")
], -1));
const _hoisted_4 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("hr", { class: "mb-8" }, null, -1));
...
const _hoisted_10 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h2", { class: "mb-4 text-xl leading-10 text-orange-400 font-semi" }, "請店家留意", -1));
const _hoisted_11 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("ol", { class: "text-sm formkit-help mb-4" }, [
  /* @__PURE__ */ createBaseVNode("li", { class: "mb-2" }, "1.若您填寫的「露天帳號」不符合露天市集規範，或未通過「露天」及「支付連 (由拍付國際資訊股份有限公司營運)」實名認證審查，商品將無法搬家至露天市集，後續將另行通知店家確認是否更換露天帳號或實名認證補件事宜。 "),
  /* @__PURE__ */ createBaseVNode("li", { class: "mb-2" }, "2.搬家商品之「露天運送方式」將儘量與商店街原設定值一致，若因兩個平台之規範不同以致設定值無法同步，將套用店家申請搬家時所選擇的「運送方式」。"),
  /* @__PURE__ */ createBaseVNode("li", { class: "mb-2" }, "3.搬家商品之「露天結帳方式」將統一設定為：ATM、信用卡一次付清、Pi 拍錢包、信用卡0利率(3期/6期/12期)。"),
  /* @__PURE__ */ createBaseVNode("li", { class: "mb-2" }, "4.搬家商品之「商品內容及圖片」將儘量與商店街原設定值一致，但有可能因兩個平台之規範不同，以致部分商品無法搬家到露天，或僅能搬移部分商品內容。"),
  /* @__PURE__ */ createBaseVNode("li", { class: "mb-2" }, [
...
const _hoisted_23 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { class: "block mb-1 text-sm font-bold text-neutral-700 dark:text-neutral-300 formkit-label" }, "信用卡號 (限台灣核發之信用卡)", -1));
const _hoisted_24 = { style: { "display": "flex", "flex": "0 0 30%", "gap": "6px" } };
const _hoisted_25 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { class: "block mb-1 text-sm font-bold text-neutral-700 dark:text-neutral-300 formkit-label" }, "有效截止月年", -1));
const _hoisted_26 = { style: { "display": "flex", "flex": "0 0 30%", "gap": "6px" } };
const _sfc_main = {
  __name: "AppRuten",
  setup(__props) {
    const urlParams = new URLSearchParams(window.location.search);
    const btype = urlParams.get("btype") || "";
    const goAttachment = ref$1("false");
    const caseId = ref$1("");
    const caseTitle = ref$1("");
    const caseIntro = ref$1("");
    const casePrice = ref$1(0);
    const caseContractNo = ref$1("");
    const ruten_account = ref$1("");
    const isShowDescImage = ref$1(false);
    const boss_mobile = ref$1("");
    const shipway = ref$1("");
    ...
    const token2 = ref$1("");
    const credit_type = ref$1("VSA");
    const credit_cd01 = ref$1("");
    const credit_cd02 = ref$1("");
    const credit_cd03 = ref$1("");
    const credit_cd04 = ref$1("");
    const expire_month = ref$1("");
    const expire_month_list = [{ value: "", label: "請選擇" }].concat(Array.from({ length: 12 }, (v, i) => (i + 1).toString().padStart(2, "0")).map((v) => {
      return { value: v, label: v };
    }));
    const expire_year = ref$1("");
    const expire_year_list = [{ value: "", label: "請選擇" }].concat(Array.from({ length: 10 }, (v, i) => i + 2024).map((v) => {
      return { value: v, label: v };
    }));
    const credit_cvv2no = ref$1("");
    ...
    const moveToCd02 = (text2, e) => {
      if (text2.length >= 4) {
        document.getElementById("credit_cd02").focus();
      }
    };
    const moveToCd03 = (text2, e) => {
      if (text2.length >= 4) {
        document.getElementById("credit_cd03").focus();
      }
    };
    const moveToCd04 = (text2, e) => {
      if (text2.length >= 4) {
        document.getElementById("credit_cd04").focus();
      }
    };
    async function handleDataSubmit(formData) {
      ...
      if (res.token) {
        token2.value = res.token;
        return true;
      }
      return false;
    }
    async function handleCreditSubmit(formData) {
      if (formData.delta !== 1) return false;
      const res = await postCreditCard(token2.value, formData.currentStep.value);
      if (res.msg) alert(res.msg);
      if (res.href) {
            ...
            createVNode(_component_FormKit, {
              ...
              default: withCtx(() => [
                      ...
                      createBaseVNode("div", _hoisted_13, [
                        createBaseVNode("div", _hoisted_14, [
                          createVNode(_component_FormKit, {
                            ...
                            "onUpdate:modelValue": _cache[1] || (_cache[1] = ($event) => ruten_account.value = $event),
                            ...
                            help: withCtx(() => [
                              ...
                              }, toDisplayString(isShowDescImage.value ? "關閉說明" : "如何查詢露天帳號？"), 1)
                            ]),
                            _: 1
                ...
                createVNode(_component_FormKit, {
                  ...
                  name: "payment",
                  "next-label": "付款",
                  "before-step-change": handleCreditSubmit
                }, {
                  stepPrevious: withCtx(() => []),
                  default: withCtx(() => [
                    _hoisted_23,
                    createBaseVNode("div", _hoisted_24, [
                      createVNode(_component_FormKit, {
                        ...
                        name: "credit_type",
                        options: { "VSA": "VISA", "MST": "MASTER", "JCB": "JCB" },
                        validation: "required"
                      }, null, 8, ["value"]),
                      createVNode(_component_FormKit, {
                        type: "text",
                        value: credit_cd01.value,
                        id: "credit_cd01",
                        name: "credit_cd01",
                        maxlength: "4",
                        onInput: moveToCd02,
                        style: { "width": "40px" },
                        validation: "required|matches:/^\\d{4}$/",
                        "validation-messages": { required: "此為必填欄位", matches: "格式錯誤" }
                      }, null, 8, ["value"]),
                      createVNode(_component_FormKit, {
                        type: "text",
                        value: credit_cd02.value,
                        id: "credit_cd02",
                        name: "credit_cd02",
                        maxlength: "4",
                        onInput: moveToCd03,
                    ...
                    createBaseVNode("div", _hoisted_26, [
                      createVNode(_component_FormKit, {
                        label: "月",
                        type: "select",
                        value: expire_month.value,
                        name: "expire_month",
                        options: unref(expire_month_list),
                        validation: "required"
                      }, null, 8, ["value", "options"]),
                      createVNode(_component_FormKit, {
                        label: "年",
                        type: "select",
                        value: expire_year.value,
                        name: "expire_year",
                        options: unref(expire_year_list),
                        validation: "required"
                      }, null, 8, ["value", "options"]),
                      createVNode(_component_FormKit, {
                        label: "CVV",
                        help: "卡片背面簽名欄上最後3碼數字",
                        type: "text",
                        value: credit_cvv2no.value,
                        name: "credit_cvv2no",
                        maxlength: "3",
                        validation: "required|matches:/^\\d{3}$/",
                        "validation-messages": { required: "此為必填欄位", matches: "格式錯誤" }
                      }, null, 8, ["value"])
                    ])
                  ]),
...
const App = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-170cbd04"]]);
